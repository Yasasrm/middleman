...
app.get("/status", (req, res) => {
  isUnauthorized(req).then(function (auth) {
    if (auth) {
      return res.status(401).send({ status: "Unauthorized Service call" });
    } else {
      try {
        let jsnObj;
        (async () => {
          ...
          await fs.unlink(
            ...
            function (err, data) {
              if (err) {
                ...
                return res
                  .status(200)
                  .send({ status: "All records are deleted!" });
              }
            }
          );
        })();
...
async function isUnauthorized(req) {
  let auth = new Promise(function (resolve) {
    try {
      let jsnObj;
      (async () => {
        await fs.readFile(
          "./database/auth/basic-auth.json",
          "utf8",
          function (err, data) {
            if (err) {
              console.log("err: " + err);
              ...
              jsnObj = JSON.parse(data);
              if (!jsnObj.authEnabled) {
                resolve(false);
              } else if (req.headers.authorization == jsnObj.authString) {
                resolve(false);
              }
            }
            resolve(true);
